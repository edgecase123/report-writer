...
class HtmlTableRenderer implements RendererInterface
{
    ...
    public function renderBand(string $type, ?int $level, $context): string
    {
        ...
        // We don't return per-band HTML anymore — it's all accumulated
        return '';
    }
    ...
    private function renderGroupHeader(int $level, array $context): void
    {
        ...
        // Close tbody if open (end previous group)
        if ($this->tbodyOpened) {
            $this->output .= "</tbody>\n";
            $this->tbodyOpened = false;
        }

        // Render <thead> only on first group header (level 0)
        if ($level === 0 && !$this->theadRendered) {

            $sampleRecord = $context['firstRecord'] ?? current($this->getSampleRecords()) ?? null;
        ...
        // Group title row (spans all columns)
        $groupValue = $context['firstRecord']['category'] ?? 'Unknown Group'; // adjust field name
        $colCount = 5; // temporary — we'll compute properly later

        $this->output .= "<tr class=\"group-header\">\n";
        $this->output .= "<td colspan=\"{$colCount}\"><strong>Group: {$groupValue}</strong></td>\n";
        $this->output .= "</tr>\n";

        // Open tbody for this group's details
        $this->output .= "<tbody>\n";
