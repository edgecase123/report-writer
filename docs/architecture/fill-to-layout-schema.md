---
title: Fill-to-Layout Schema (v1 target spec)
updated: 2026-08-22
status: normative target — current library implements a subset
mirrors: https://github.com/edgecase123/report-writer/tree/main/docs/architecture
---

> **Status.** Normative specification for the v1 layout stage — describes the contract fill emits and the behaviors layout must exhibit. The current `writer/` library implements a proper subset: single-pass pagination with fit/place, one splittable single-element text band per page break, and `Flattener`-based subreport inlining that discards the parent-linkage this spec requires. **Not yet shipped:** fragment identity on continuations, keep-together / keep-with-next / group-keep-with-first-detail rules, forced page breaks, page headers/footers, subreport-as-container pagination with preserved ownership, stretchable elements with measured heights, layout diagnostics with trace IDs.
>
> For what the code actually does today, see [Overview → Architecture](../01-overview/architecture.md) and the pipeline pages under [Pipeline](../04-pipeline/) (planned). The gap between this spec and the current impl is the v1 layout-engine work.
