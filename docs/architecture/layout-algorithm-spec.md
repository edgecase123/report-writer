---
title: Layout Algorithm Specification v1
updated: 2026-08-22
...
> **Status.** Draft normative specification for the layout stage. The current `writer/` library implements a proper subset (see [Overview → Architecture](../01-overview/architecture.md) for what's shipped). The MUST/SHOULD language below is aspirational for a v1 implementation — treat this as the target, not the current behavior.
