---
title: Canonical Layout Test Cases (future)
updated: 2026-08-22
status: v1 target test cases — not yet writable against current library
mirrors: https://github.com/edgecase123/report-writer/tree/main/docs/architecture
---

> **Status.** These two canonical test cases assert the v1 output shape defined in [`layout-algorithm-spec.md`](layout-algorithm-spec.md) — fragment IDs, `continues_on_next_page` flags, `child_band_fragments` with preserved `parent_element_instance_id`, unified pagination across parent + subreport flow. The current library does not produce this output shape (it emits flat `PositionedElement` lists per page and `Flattener`-inlines subreports before layout). These tests become writable once the v1 layout engine lands; keep them here as the target contract to build toward.
